  ...
  app.controller('appController', function($scope, $http){
    ...
    $scope.toggleMode = function() {
      ...
      if ($scope.isCelsius === false) {
        $scope.temp = (($scope.curTemp * 9/5) + 32).toPrecision(precision);
        $scope.tempMin = (($scope.minTemp * 9/5) + 32).toPrecision(precision);
        $scope.tempMax = (($scope.maxTemp * 9/5) + 32).toPrecision(precision);
        $scope.unit = 'F';
      } else {
        $scope.temp = ($scope.curTemp).toPrecision(precision);
        $scope.tempMin = ($scope.minTemp).toPrecision(precision);
        $scope.tempMax = ($scope.maxTemp).toPrecision(precision);
        $scope.unit = 'C';
      }
    };
